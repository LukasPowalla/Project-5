...
void solarsystem::insert_data(vector<planet> vec, mat &ma){
    for(int i=0; i<vec.size(); i++){
       all_planets[i] = vec[i];
        ma(i,6)=all_planets[i].mass;

        for(int k=0; k<3;k++){
            ma(i,k)=all_planets[i].position[k];
            ma(i,k+3)=all_planets[i].velocity[k];
        }
    }
}
...
void solarsystem::synctroniz(vector<planet> vec, mat &ma){
    ...
    for(int j=0; j<n;j++){
        all_planets[j] = vec[j];
    for (int i = 0; i < 3; ++i){
       all_planets[j].position[i] =  ma(j,i);
       all_planets[j].velocity[i] = ma(j,i+3);
}
}
